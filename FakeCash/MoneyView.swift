...
struct WhiteCardSection: View {
    ...
    var body: some View {
        ZStack(alignment: .top) {
            Color.white.cornerRadius(28, corners: [.topLeft, .topRight])
            ...
            ForEach(items, id: \.1) { icon, label, sub in
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        Text(label).font(.system(size: 17, weight: .semibold)).foregroundColor(.black)
                        Text(sub).font(.system(size: 14)).foregroundColor(Color(hex: "888888"))
                    }
                    Spacer()
